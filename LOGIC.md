...
| ⚠️ **정산 달력 보기** | **완전 목업** — 파일 내 하드코딩 배열, DB 미연동. 🆕 **출금 신청 UI로 컨셉 확장 프로토타입 제작됨**(건당수수료 구간 시각화, 별도 Artifact) — 아직 관리자 작업 우선이라 목업 단계 유지, 실개발은 라이더 착수 시 |
...
🔧 **[정정] 건당(이체)수수료 계산방식 — 기존 `fee_per_tx_short/long` 단일값 모델에서 변경.** 2026-07-13 라이더 앱 "출금 신청 정산 달력" 목업 논의로 확정:
...
- **단위는 출금 1건이 아니라 "배달 주문 1건당"** — 즉 하나의 출금 신청 안에 최근분(80원 적용 주문)과 오래된분(40원 적용 주문)이 **섞여서 각각 합산**될 수 있음. 기존처럼 `accrued_days` 하나로 전체 출금에 단일 수수료를 매기는 방식이 아님.
- 계산: `총수수료 = (7일 이내 주문 건수 × 80원) + (7일 지난 주문 건수 × 40원)`, `실지급 = balance − reserve_amount − 총수수료`.
- 화면에는 "40원 구간 N건=₩x / 80원 구간 M건=₩y / 총 수수료 ₩z / 예상 출금액 ₩w" 형태로 분리 표기.
- **현재는 라이더 앱 목업(프로토타입)에만 반영, `Withdrawal`/`WithdrawalConfig` 실제 도메인 로직은 아직 구 모델(단일 accrued_days 기준) 그대로** — 라이더 쪽 실제 개발 착수 시 `Withdrawal::applyForRider()`·`RiderWallet::previewWithdrawal()` 재작성 필요(§7).
- 대리점별 override 여부는 기존과 동일하게 **본사 전역값**으로 유지(§2 결제수수료 관련 논의와 별개로, 이 건당수수료율 자체가 대리점별로 달라진다는 이야기는 없었음 — 재확인 시 명시 예정).
...
18. 🔧 **출금 건당수수료 계산 모델 교체** — 단일 `accrued_days` 기준 → 배달 주문별 "오늘로부터 경과일" 기준 age-bucket 합산 모델로. `Withdrawal::applyForRider()`·`RiderWallet::previewWithdrawal()` 재작성 (§5.4)

**🆕 라이더 앱 — 재설계와 무관하게 원래 남아있던 미구현분 (§3, 관리자 작업 후 착수)**
19. 정산 달력 보기 — 완전 목업, DB 연동 필요. 출금 신청과 결합한 신규 컨셉(건당수수료 구간 시각화) 프로토타입 완료, 정식 개발 시 반영
20. 정산 상세("샘플") — 완전 목업, DB 연동 필요
21. 홈 "이번 달 정산 합계" 카드 — 하드코딩 제거, 실제 집계로 교체
...
| 2026-07-13 | 관리자 작업 일시 보류(갑 미팅 결과 대기), **라이더 앱 착수 준비** — 정산 달력·상세("샘플")·홈 정산합계 카드가 여전히 목업임을 재확인. 출금 신청용 "정산 달력" 컨셉 목업(Artifact) 제작하며 **건당(이체)수수료 모델 정정** — 기존 "마지막 출금 후 경과일 단일값" → "오늘로부터 각 배달 주문의 경과일" 기준, **주문 건별로** 7일 이내 80원/7일 지남 40원이 섞여 합산되는 방식으로 확정(§5.4). 플랫폼(쿠팡·배민)의 주간 정산 주기 때문에 대리점이 아직 못 받은 최근분을 미리 주면 수수료가 더 붙는 구조. 라이더 도메인 로직(`Withdrawal`/`RiderWallet`) 재작성은 향후 착수(§7 #18) |
