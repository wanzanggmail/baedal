...
## 펌뱅킹 보정 조회 크론 (실 연동 서버 필수)

바움의 처리결과 통보는 1분 간격 **최대 10회** 재전송 후 그친다. 그 사이 서버가 내려가 있었거나
통보 IP 가 막혀 있었으면 결과를 영영 못 받고, 출금이 `접수중` 에 갇힌다(라이더는 재신청도 못 한다).
관리자 화면의 「보정 조회」 버튼만으로는 사람이 눌러야 풀리므로 크론으로 돌린다.

```bash
crontab -e
# 10분마다 미확정 이체를 바움에 직접 조회해 확정한다.
*/10 * * * * /usr/bin/php /var/www/baedal/scripts/firm_reconcile.php >> /var/log/firm_reconcile.log 2>&1
```

- 겹쳐 돌아도 안전하다(`GET_LOCK` + 확정 자체가 멱등).
- 실 연동이 꺼진 서버(모의 게이트웨이)에서는 아무것도 하지 않는다.
- 경로는 실제 배포 경로로 바꾼다(`DEPLOY_PATH`).
