...
1. `migrate.php`로 스키마를 생성·갱신합니다.
2. `seed.php`로 기본 조직, 관리자, 공통 코드를 구성합니다.
3. 관리자 로그인 후 조직과 테스트 데이터를 운영값으로 정리합니다.
4. 정산 엑셀 암호를 시스템 관리에서 설정합니다.
5. 운영 서버에서는 `seed.php`의 웹 접근을 차단하거나 파일을 제거합니다.
...
3. 백업이 있으면 우선 복원합니다.
4. 백업이 없으면 `php migrate.php` 후 `php seed.php`를 실행합니다.
...
- `seed.php`와 마이그레이션 도구의 웹 접근을 차단합니다.
