...
바움에 우리 IP 를 `13.209.173.109` 로 갱신 등록 → **개발 API 전부 통과**.
어제까지 403 이던 토큰 발급이 200 + access_token 으로 바뀌었다.
...
**엔드포인트 전수 실 검증 완료 (2026-08-31)** — 아래 넷 모두 개발 서버에서 실제 호출로 확인.

| 기능 | 결과 |
|---|---|
| 계좌이체 접수 | ✅ 100원 접수 → RECEPTION |
| 이체 정보 조회 | ✅ RECEPTION → SUCCESS 확인 |
| 계좌이체 취소 | ✅ 접수 직후 취소 → CANCELLED |
| 통보 URL 관리 | ✅ 등록·조회·삭제 |

- [x] **처리결과 통보 발신 IP = `221.168.32.176`** — 실 통보를 받아 알아냈다(2026-08-31).
      `firm_config.noti_allow_ips` 에 등록함. 이 IP 밖의 통보는 거절된다.
      (개발 서버 통보의 IP. 운영 서버 통보 IP 는 실 연동 시 재확인 필요할 수 있다.)
- [ ] 포켓코드가 개발·운영 공통인지 (한 개만 받았다)

## 9-A. 실 연동(라이브) 전 점검 — 2026-09-19

실 이체 테스트 전에 코드를 전수 점검해 **고친 것 4건**과 **운영이 확인할 것 5건**을 정리했다.

### 고친 것

| # | 문제 | 왜 위험한가 | 조치 |
|---|---|---|---|
| 1 | **같은 출금 건을 두 번 이체할 수 있었다** | 상태를 접수 «후» 에 바꿔서, 관리자 두 번 클릭이나 자동출금과 수동 확정이 겹치면 두 프로세스가 같은 건을 집어 **각각 다른 `transactionId` 로** 접수했다. id 가 다르니 바움도 못 막는다 → **같은 돈이 두 번 나간다.** 지갑은 한 번만 깎여 차이도 늦게 드러난다 | 접수 **전에** 조건부 UPDATE 로 `transferring` 을 선점(claim). 진 쪽은 큐에서 빠지고 「이미 이체를 시작한 건」으로 보고 |
| 2 | **보내지도 않은 이체를 「접수중」으로 기록** | `submit()` 이 예외를 던지는 지점은 전부 HTTP 호출 **이전**(은행코드 미등록·배치 상한·토큰 발급 실패·암호화 실패)인데, 배치 경로가 이를 «응답 불명» 으로 보고 최대 100건을 접수중으로 남겼다. 나가지도 않은 이체를 기다리며 라이더가 영영 묶인다 | 예외는 **실패**로 처리해 재시도 가능하게. 통신 장애는 예외가 아니라 `ok=false` 로 오므로 기존의 보수적 처리(접수중 유지)가 그대로 남는다 |
| 3 | 보정 조회가 «그런 거래 없음» 을 못 풀었다 | `RESOURCE_NOT_EXISTS` 를 단순 오류로 세고 넘어가 접수중이 영구히 남았다 | 바움이 «없다» 고 하면 전송되지 않은 것 → 실패로 확정해 재시도를 연다 |
| 4 | 보정 조회에 **금액 대조가 없었다** | 웹훅은 금액이 다르면 확정을 멈추는데 보정 조회는 그냥 확정했다 — 두 경로의 기준이 달랐다 | 조회 응답의 `amount` 를 장부와 대조, 다르면 확정하지 않고 사람이 보게 남긴다 |

덤으로 **0원 이체를 보내기 전에 막고**, 「접수중인데 이체 장부에 행이 없는」 고아 건을
보정 조회가 세어서 보고한다(자동 처리하지 않는다 — 실제로 나갔을 수 있어 사람이 확인해야 한다).

### 추가 점검 — 웹훅·순차 이체·제한 시간 (2026-09-19 2차)

**🐛 중간 상태 통보가 출금을 「실패」로 만들고 있었다 (가장 위험).**
바움은 `RECEPTION → PROGRESS → NEED_CHECK → SUCCESS` 로 **중간 상태도 통보**한다. 웹훅 처리는
「SUCCESS 가 아니면 실패」로 갈라서, **진행 중인 이체가 출금 실패로 찍혔다.** 실패는 재시도 대상이라
관리자가 다시 확정을 누르면 **같은 돈이 두 번 나갈 수 있었다.** 확정 상태(SUCCESS/FAILED/CANCELLED)
일 때만 원본 장부를 건드리도록 고쳤다(`applyResult` 안에도 같은 가드를 하나 더 둠).

| 점검 | 방법 | 결과 |
|---|---|---|
| 웹훅 전 구간 | 실 dev 키로 본문을 암호화해 `FirmWebhook::handle()` 에 넣고 트랜잭션 롤백 | **20/20** — 완료+지갑 101,500원 차감, 재전송 멱등, 실패·취소 처리, 금액 불일치 보류, 중간 상태 대기, 허용 밖 IP 403, 평문 400, 다른 키 400, 미매칭 200, 입금(+) 기록, 이력에 계좌 원문 없음 |
| 웹훅 HTTP 진입점 | `curl` 로 `/firm/noti.php` | GET 405 · 설정 미완료 POST 503 `{"success":false}` |
| 순차 이체 60건 | 한 번에 확정 | 60/60 완료, 오류 0, 12.1초(건당 202ms — 로컬에서 원격 DB 기준) |
| 같은 대리점 동시 확정 4건 | 프로세스 4개가 같은 대리점·라이더를 동시에 | 교착·타임아웃 없음. 대리점 지갑 행에서 **순서대로 처리**(각 0.25초). 잔액 변화 0 |
| 이체 제한 시간 | 경계값 12종 + 실제 차단 | **20/20** |

### 이체 제한 시간 (신규)

`firm_config.block_from` / `block_to` (기본 **23:30~00:30**, HH:MM, 비우면 제한 없음).
관리자 → 펌뱅킹 연동 화면에서 바꾼다. 자정을 넘는 구간도 계산한다.

제한 시간에는 **접수 자체를 하지 않고 신청을 `pending` 그대로 둔다.** 실패로 만들면 라이더가
재신청도 못 하기 때문이다(관리자가 재시도하거나 반려해야 풀린다). 시간이 지난 뒤 그대로 확정하면 된다.

⚠️ 기본값 23:30~00:30 은 **우리가 정한 보수적인 값**이다. 바움 매뉴얼에 정확한 시간대가 없으니
(`TRANSFER_RESTRICTED_TIME` 오류코드만 있음) 실제 제한 시간을 확인해 맞추는 게 좋다.
### 크론 등록 (필수)

보정 조회가 **관리자가 버튼을 눌러야만** 돌았다. 통보가 유실되면 아무도 안 누르는 동안
출금이 접수중에 갇힌다(라이더는 재신청 불가). CLI 를 추가했으니 실서버 크론에 넣는다.

```
*/10 * * * * /usr/bin/php /var/www/baedal/scripts/firm_reconcile.php >> /var/log/firm_reconcile.log 2>&1
```

`GET_LOCK` 으로 겹침을 막고, 실 연동이 꺼져 있으면 아무것도 하지 않는다.

### 라이브 테스트 전 확인할 것

1. **운영 통보 IP** — `firm_config.noti_allow_ips` 에 들어 있는 `221.168.32.176` 은 **개발 서버**
   통보 IP 다. 운영 발신 IP 가 다르면 통보가 전부 403 으로 거절돼 **모든 이체가 접수중에 갇힌다.**
   바움에 운영 IP 를 확인해 추가하거나, 첫 테스트 동안은 비워 두고(복호화 성공이 인증 역할) 실제
   수신 IP 를 확인한 뒤 채운다.
2. **운영 통보 URL 등록** — 운영 환경에도 `https://{운영도메인}/firm/noti.php` 를 등록했는지.
   개발에 등록한 것은 운영에 적용되지 않는다.
3. **운영 포켓코드** — 개발과 같은 `8782202567` 로 저장돼 있다. 운영 포켓이 다르면
   `POCKET_NOT_EXISTS` 로 접수가 거절된다.
4. **`APP_ENC_KEY`** — 웹서버마다 같아야 한다. 다르면 라이더 계좌번호가 빈 문자열로 복호화돼
   **전액 접수 거절**된다(엉뚱한 계좌로 가지는 않는다 — GCM 이라 위조 복호화가 불가능).
5. **이체 제한 시간** — `TRANSFER_RESTRICTED_TIME` 오류가 있다. 은행 점검 시간대(보통 23:30~00:30)에
   자동출금이 돌면 무더기 실패한다. 첫 테스트는 영업시간에, 금액은 100원 단위로.

### 아직 모의(mock)인 경로 — 실 이체가 아니다

`일일지급`(`DailyPayout`)과 `대리점 인출`(`AgencyPayout`)은 펌뱅킹이 아니라 **오픈뱅킹
게이트웨이**를 타는데, `OpenBankingGatewayFactory::make()` 가 지금도 **항상 모의**를 돌려준다.
그 화면에서 「지급」을 누르면 **돈은 나가지 않는데 지급 완료로 기록된다.** 라이더 출금
(주정산·선정산 자동출금)은 전부 펌뱅킹을 타므로 영향이 없다.
...
## 16. 실 서버 전수 검증 (2026-08-31)

허용 IP 가 열린 뒤 개발 서버에서 **전 기능을 실제로 호출**해 확인했다.

| 단계 | 결과 |
|---|---|
| 토큰 발급 | ✅ 200 + access_token (유효 ~24h) |
| 잔액 조회 | ✅ 기본포켓 1천만원 |
| 예금주 조회 | ✅ `100280001225` → "테스트" (A0000) |
| 예금주 조회 실패 | ✅ `9999999999999` → PE902 |
| 이체 접수 | ✅ 100원 → RECEPTION |
| 이체 정보 조회 | ✅ 7초 뒤 SUCCESS (A0000) |
| 이체 취소 | ✅ RECEPTION 상태에서 취소 → CANCELLED |
| **통보 수신** | ✅ `/firm/noti.php` 로 SUCCESS 통보 도착·복호화·매칭·상태갱신 |

**통보 왕복이 실제로 돌았다** — 접수한 100원이 SUCCESS 되자 바움이 1분 내에
운영 서버 `/firm/noti.php` 로 통보를 보냈고, 우리가 복호화→매칭→`firm_transfers`
상태를 SUCCESS 로 갱신했다. 발신 IP `221.168.32.176` 도 이때 확인.

**사소한 것** — `transfer-info` 응답에는 `success` 필드가 없어 우리 `call()` 의 `ok`
판정이 false 로 나온다(데이터는 정상). 조회 결과는 `ok` 가 아니라 `status` 로 본다.

검증 뒤 통보 URL 은 삭제하고 `driver=mock` 으로 되돌렸다. 실 연동을 켤 때
다시 등록하면 된다.
