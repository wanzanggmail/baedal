...
### 3차 점검 — 잔액 초과 이체 (2026-09-19)

**🐛 대리점 잔액보다 많이 내보낼 수 있었다.** 지갑 차감은 **이체가 확정될 때**(웹훅) 일어나는데
잔액 검사는 **접수 전에** 하므로, 한 번에 여러 건을 확정하면 전부 같은 잔액을 보고 통과한다.
재현: 25만원 지갑에서 10만원짜리 5건을 한 번에 확정 → **5건 전부 나가고 잔액 −25만원.**

고친 방식 — **가용액 = 지갑 잔액 − 이미 접수중인 금액 − 이번 배치에서 앞서 잡은 금액.**
같은 조건에서 2건만 나가고 3건은 「대리점 잔액 부족(가용 5만원 …)」으로 거절된다.
동시에 두 곳에서 확정해도 어긋나지 않도록 **이체 실행 전체를 전역 잠금으로 직렬화**했다
(`GET_LOCK('wd_exec_…')`, 30초 대기 · 못 잡으면 「다른 이체 처리가 진행 중」). 이체는 하루 몇
차례라 전역으로 충분하다 — 처리량이 문제가 되면 대리점 단위로 쪼갠다.

같이 확인한 것(전부 정상): 접수중 건은 반려 대상에서 제외 · 접수중·실패 동안 라이더 재신청 차단 ·
250건 배치가 100/100/50 으로 나뉘며 `transactionId` 키 유지 · 장부 없는 접수중 탐지.
