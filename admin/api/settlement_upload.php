...
try {
    ...
    if ($detectedKind === 'weekly') {
        ...
        $weeklyIns  = [];
        if ($weeklyRows === []) {
            // 쿠팡 주정산서 — **시간제보험만** 반영한다(2026-08-22 갑 확정: "쿠팡은 프로모션
            // 반영 안 할꺼야"). 그래서 아래 두 가지는 **일부러 읽지 않는다**:
            //   · 「협력사 자체 미션」 시트(라이더별 165만원) — 갑이 반영 대상이 아니라고 확정
            //   · 종합 시트의 「프로모션」 총액(5.2만원) — 라이더별 내역이 없고 위와 값도 다름
            //   · 라이더별 정산 표 — 일일과 컬럼 구조가 같아 그대로 읽으면 한 주치가 하루치로
            //     저장된다(실측 1,730건). 판별(detectKind)로 걸러진 뒤에도 손대지 않는다.
            // ⚠️ "프로모션이 빠졌다"고 나중에 채워 넣지 말 것 — 누락이 아니라 확정된 정책이다.
