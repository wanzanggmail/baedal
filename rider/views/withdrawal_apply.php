...
<div class="card card-flush shadow-sm mb-4">
	<div class="card-header border-0 min-h-auto align-items-center gap-2 pt-3 pb-0">
		<h2 class="card-title fw-bold fs-5 my-0">출금 신청</h2>
		<span class="text-gray-500 fs-8 text-end">달력에서 출금할 기간을 선택하세요</span>
	</div>
	<div class="card-body pt-2 pb-3">
		<?php if ($flashOk !== '') : ?>
		<div class="alert alert-success fs-7 p-3 mb-3"><?= htmlspecialchars($flashOk, ENT_QUOTES, 'UTF-8') ?></div>
		<?php endif; ?>
		<?php if ($flashErr !== '') : ?>
		<div class="alert alert-danger fs-7 p-3 mb-3"><?= htmlspecialchars($flashErr, ENT_QUOTES, 'UTF-8') ?></div>
		<?php endif; ?>

		<?php if ($unwithdrawn === []) : ?>
		<div class="alert bg-light-warning fs-7 py-2 px-3 mb-0">출금 가능한 정산 내역이 없습니다.</div>
		<?php else : ?>
		<div class="alert bg-light-primary fs-8 py-2 px-3 mb-0">
			출금은 <strong>오래된 정산분부터 순서대로</strong> 나갑니다. 날짜를 누르면 <strong>그 날짜까지</strong> 신청됩니다.
		</div>
		<?php endif; ?>
	</div>
</div>
