...
🆕 **(2026-08-31 갑 지시) 정산수수료 구간별 배분** — 본사·총판 몫 **모두 배달 건당 정액(원)**이며, 「기준 미만/기준 이상」 두 구간에 각각 다르게 매긴다: `hq_fee_short`/`hq_fee_long`(본사 몫) · `dist_fee_short`/`dist_fee_long`(총판 몫). **대리점 몫 = 대행수수료 − 본사 − 총판**(나머지 전부). **본사 몫(건당)의 하한**은 별도 컬럼이 아니라 **「대행수수료 설정」의 최저 금액**(`deduction_global_config.agency_fee_min_short`/`agency_fee_min_long`, `AgencyFeeConfig::minimums()`)을 구간별로 그대로 쓴다. 🔄 **2026-09-06 갑: 하한 대상은 `hq + tax + dev` 합계**(본사·세무대리·개발사) — 저장 시 `hq_fee_short+tax_fee_short+dev_fee_short < min_short || hq_fee_long+tax_fee_long+dev_fee_long < min_long`이면 거부(InvalidArgumentException→422). 0이면 하한 없음. 대리점별 설정이지만 **편집은 본사만**(대리점은 조회).
...
**`message_queue`**(2026-09-03 확장) — `channel` enum 에 **`lms`** 추가(본문이 `sms_max_bytes` 초과면 자동 전환). `agency_id`(과금 대상 대리점 — 라이더 소속에서 자동 결정, NULL이면 과금 안 함)·`charged_amount`(실제 과금액)·`template_code`(알림톡 승인 템플릿). 발송 **성공 시에만** 채널 단가만큼 대리점 지갑 → 본사로 이체(`agency_wallet_ledger` reason `msg_fee_up`/`msg_fee_in`, `MessageQueue::chargeSend`). 실패 건은 과금하지 않는다. `message_send_logs` 도 같은 확장(`lms`·`charged_amount`).

**`message_queue.fallback_from`**(2026-09-02) — 알림톡 실패 **SMS 대체발송**이면 원본 알림톡 id. NULL=일반. `messaging_config.alimtalk_fallback_sms`(기본 켬)가 켜져 있고 알림톡이 카카오 수신불가(미설치·차단·미사용자 등, `MessageQueue::FALLBACK_REASONS`)로 실패하면 같은 내용을 SMS 로 새 행 생성·즉시 발송(`MessageQueue::maybeFallbackToSms`). 원본당 1건만(중복 방지), SMS 는 다시 대체발송 안 됨.

🆕 **`message_send_logs`**(2026-09-01) — 발송 로그(**append-only**). `reason_code`(2026-09-02, 실패 분류 코드 — 대체발송 판정) 포함. 큐 행은 재발송 시 상태가 덮이므로, 발송 시도마다 1행씩 영구 기록. `message_id`(nullable, message_queue.id)·`channel`·`rider_id`·`recipient_name/phone`·`title`·`content`·`status`(sent/failed)·`provider/provider_ref/error`·`attempted_by`(admins.id)·`attempted_at`. `MessageQueue::send()`가 성공·실패 양쪽에 `logAttempt()`로 기록(로그 실패는 발송 무시). 조회: `MessageQueue::listLogs()/logCounts()`(기간·채널·상태·검색 필터), 화면 `system/messages` 하단 「발송 로그」 카드(API `?logs=1`). 큐가 정리돼도 로그는 남는다.

🆕 **`alimtalk_templates`**(2026-09-03) — 알림톡 상황별 템플릿. `event_key`(settlement_statement·withdrawal_done·withdrawal_failed·notice, UNIQUE)·`name`·`template_code`(카카오 승인 코드)·`title`·`content`(치환변수 `#{키}`)·`variables`(본문에서 자동 추출)·`channel_policy`(alimtalk_first=알림톡 우선+실패 시 문자 대체 / sms_only=문자만)·`is_active`. `AlimtalkTemplate`(forEvent·save·render·plan·seedDefaults). 화면 `system/alimtalk-templates`(본사 super). 발신 프로필키는 계정 단위라 `messaging_config.alimtalk_channel` 에 둔다.

🆕 **`statement_links`**(2026-09-01) — 모바일 명세서 공개 링크 토큰. 라이더가 파일 대신 **링크**로 명세서를 연다(알림톡). `token`(40 hex, unique)·`rider_id`·`period_from/to`·`created_by`·`created_at`·`expires_at`(기본 +90일)·`view_count`·`last_viewed_at`. `StatementLink`(create[같은 라이더·기간 유효 링크 재사용]·resolve[만료 체크]·markViewed·url[절대 URL, env `PUBLIC_BASE_URL` 우선]). 공개 페이지 `rider/p/statement.php?t=토큰`(로그인 불필요·자체완결 경량 HTML·noindex). 일정산 자동 알림톡 본문에 링크 자동 첨부(`RiderStatement::enqueueDailyStatements`). 링크 도메인·유효기간은 `messaging_config` 설정을 따름.

🆕 **`messaging_config`**(2026-09-02) — 알림톡·문자 발송 설정(전역 단일 행, 본사). `sender_phone`(발신번호)·`alimtalk_channel`(발신 프로필/채널)·`statement_template`(명세서 템플릿 코드)·`public_base_url`(명세서 링크 기본 도메인)·`link_ttl_days`(링크 유효기간, 기본 90)·`alimtalk_fallback_sms`(알림톡 실패 시 SMS 대체발송, 기본 1)·**`price_alimtalk`/`price_sms`/`price_lms`**(건당 단가, 기본 10/10/50원)·**`sms_max_bytes`**(기본 90, EUC-KR 기준 초과 시 LMS). `MessagingConfig`(get·save·publicBaseUrl·linkTtlDays·alimtalkFallbackSms). 화면 `system/messages` 상단 「알림톡·문자 설정」 카드. ⚠️ 발송사 자격증명(id/pw)은 여기 저장하지 않음(연동 시 `Crypto` 암호화 별도).
